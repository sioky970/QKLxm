...
func (mon *Monitor) Start() {
	if !atomic.CompareAndSwapUint32(&mon.running, 0, 1) {
		logger.Warn("火币数据监控已在运行中")
		return
	}
	go mon.runHealthCheck()
	go mon.runStatusReport()
	logger.Info("火币数据监控启动成功")
}

// Stop 停止监控
func (mon *Monitor) Stop() {
	mon.stopOnce.Do(func() {
		if atomic.LoadUint32(&mon.running) == 1 {
			close(mon.stopChan)
			atomic.StoreUint32(&mon.running, 0)
			logger.Info("火币数据监控已停止")
		}
	})
}
...
func (mon *Monitor) runHealthCheck() {
	...
	for {
		...
		case <-mon.stopChan:
			logger.Info("健康检查监控goroutine已退出")
			return
		}
	}
}
...
func (mon *Monitor) runStatusReport() {
	...
	for {
		...
		case <-ticker.C:
			mon.reportStatus()
		case <-mon.stopChan:
			logger.Info("状态报告监控goroutine已退出")
			return
		}
	}
}
