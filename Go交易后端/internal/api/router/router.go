...
func Setup() *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		auth.Use(middleware.JWTAuth())
		{
			...
			// 资金钱包模块
			auth.GET("/wallet/fund/balance", handler.GetFundWalletHandler().GetFundWalletBalance)             // 获取资金钱包余额
			auth.GET("/wallet/fund/list", handler.GetFundWalletHandler().GetAllFundWallets)                  // 获取所有资金钱包
			auth.GET("/wallet/fund/overview", handler.GetFundWalletHandler().GetFundWalletOverview)           // 获取资金钱包总览
			auth.GET("/wallet/fund/all-balances", handler.GetFundWalletHandler().GetAllWalletBalances)        // 获取所有钱包余额总览
			auth.POST("/wallet/fund/transfer-in", handler.GetFundWalletHandler().TransferToFund)              // 转入资金钱包
			auth.POST("/wallet/fund/transfer-out", handler.GetFundWalletHandler().TransferFromFund)            // 转出资金钱包
			auth.GET("/wallet/fund/transfers", handler.GetFundWalletHandler().GetFundWalletTransfers)         // 获取划转记录列表
			auth.GET("/wallet/fund/transfers/:transfer_no", handler.GetFundWalletHandler().GetFundWalletTransferDetail) // 获取划转记录详情
