...
func GetUserList(c *gin.Context) {
	logger.Info("=======================================")
	logger.Info("[GetUserList] 收到用户列表请求")
	...
	logger.Infof("[GetUserList] 请求参数: page=%d, pageSize=%d", info.Page, info.PageSize)
	...
	// 查询每个用户的现货、永续合约、交割合约和资金钱包余额
	walletService := service.GetWalletTransferService()
	fundWalletService := service.GetFundWalletService()
	...
	for _, user := range list {
		...
		// 获取资金钱包余额
		fundWallet, fundErr := fundWalletService.GetOrCreateFundWallet(uint64(user.ID), 1)
		if fundErr != nil {
			logger.Warnf("[GetUserList] 获取资金钱包失败: userID=%d, err=%v", user.ID, fundErr)
		} else if fundWallet != nil {
			// 转换余额
			fundBalance, _ := fundWallet.AvailableBalance.Float64()
			item.FundBalance = fundBalance
			logger.Infof("[GetUserList] 获取资金钱包成功: userID=%d, balance=%s", 
				user.ID, fundWallet.AvailableBalance.String())
		} else {
			logger.Warnf("[GetUserList] 资金钱包为空: userID=%d", user.ID)
		}
