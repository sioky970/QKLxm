...
func UpdateWalletBalance(c *gin.Context) {
	var req struct {
		...
		WalletType  string  `json:"wallet_type"` // spot 或 contract
	}
	...
	if walletType != "spot" && walletType != "contract" {
		response.BadRequest(c, "钱包类型只能是 spot 或 contract")
		return
	}

	if err := adminService.Wallet.UpdateBalance(req.ID, req.BalanceType, req.Amount, req.Reason, walletType); err != nil {
		response.Error(c, err.Error())
		return
	}
...
func ApproveWithdrawal(c *gin.Context) {
	...
	if err := adminService.Wallet.ApproveWithdrawal(req.ID, 0, "admin", req.Notes); err != nil {
		response.Error(c, err.Error())
		return
	}
...
func RejectWithdrawal(c *gin.Context) {
	...
	if err := adminService.Wallet.RejectWithdrawal(req.ID, 0, "admin", req.Reason); err != nil {
		response.Error(c, err.Error())
		return
	}
...
func GetChargeList(c *gin.Context) {
	...
	list, total, err := adminService.Wallet.GetRechargeListWithFilter(0, req.CurrencyID, status, req.Page, req.PageSize)
	if err != nil {
		response.Error(c, err.Error())
		return
	}
...
func ApproveCharge(c *gin.Context) {
	...
	if err := adminService.Wallet.ApproveRecharge(req.ID, 0, "admin", ""); err != nil {
		response.Error(c, err.Error())
		return
	}
...
func RejectCharge(c *gin.Context) {
	...
	if err := adminService.Wallet.RejectRecharge(req.ID, 0, "admin", req.Reason); err != nil {
		response.Error(c, err.Error())
		return
	}
