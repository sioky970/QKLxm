							...
							<view v-else class="position-list">
								<view class="position-item" v-for="pos in positions" :key="pos.id">
									<view class="position-header">
										<view class="left">
											<text class="symbol">{{ pos.currency_name || 'BTC' }}/USDT</text>
							...
							<view v-else class="pending-list">
								<view class="pending-item" v-for="order in pendingOrders" :key="order.id">
									<view class="pending-header">
										<view class="left">
											<text class="symbol">{{ order.currency_name || 'BTC' }}/USDT</text>
...
// 控盘阈值（最后2秒进入控盘阶段）
const CONTROL_THRESHOLD_SECONDS = 2
...
const onSymbolSelect = (item) => {
	...
	console.log('[Contract] 币种选择:', item.symbol)
}
...
const fetchUserAssets = async () => {
	try {
		...
		console.log('[Contract] getAllAssetsWithBalance 响应:', res)
		...
		if (usdtAsset) {
			...
			console.log('[Contract] USDT余额:', usdtBalance.value)
		} else {
			usdtBalance.value = 0
		}
		...
		if (coinAsset) {
			...
			console.log(`[Contract] ${coinName}余额:`, currentCoinBalance.value)
		} else {
			currentCoinBalance.value = 0
		}
	} catch (err) {
		console.error('[Contract] 获取资产失败:', err)
		uni.showToast({
...
const mapMicroOrderToFrontend = (order) => {
	...
	return {
		id: order.id,
		symbol: order.currency_name ? `${order.currency_name}/USDT` : 'BTC/USDT',
		side: side,
		amount: amount,
...
// 获取仓位盈亏 - 优先使用后端返回值，否则前端计算
const getPositionPnL = (position) => {
	// 如果后端返回了 unrealized_pnl，直接使用
...
const getPositionPnLRate = (position) => {
	// 如果后端返回了 pnl_rate，直接使用
	if (position.pnl_rate !== undefined && position.pnl_rate !== null) {
		return position.pnl_rate.toFixed(2)
	}
...
const calcUnrealizedPnL = (position) => {
	...
	// 获取当前价格：优先后端返回的 current_price，否则从 marketStore 获取
	let currentPrice = position.current_price
	if (!currentPrice || currentPrice <= 0) {
		const currentMarketData = marketStore.getMarketData(position.currency_id, position.legal_id)
		currentPrice = currentMarketData?.price || entryPrice
	}
...
const setupWebSocketSubscriptions = () => {
	...
	wsClient.subscribe(walletChannel, (data) => {
		console.log('[Contract] 收到余额更新推送:', data)
	...
	console.log('[Contract] WebSocket订阅完成:', { walletChannel, positionChannel, microChannel })
}
...
const handleBalanceUpdate = (data) => {
	...
	if (usdtAsset) {
		...
		console.log('[Contract] USDT余额已更新:', usdtBalance.value)
	}
	...
	if (coinAsset) {
		...
		console.log('[Contract] 币种余额已更新:', coinName, currentCoinBalance.value)
	}
...
const onSliderChange = (e) => {
	...
	marginAmount.value = usdtAmount.toFixed(2)
...
const onMarginInput = (e) => {
	...
	if (numVal > maxUsdt) {
		marginAmount.value = maxUsdt.toFixed(2)
	}
