...
const totalUsdValue = ref('0.00')
const profitData = ref({ value: 0, rate: '0.00%' })
...
const fetchAssetData = async () => {
	...
	try {
		...
		// 更新总资产（使用9位有效数字格式化）
		totalBalance.value = formatBalanceWith9Digits(data.total_balance)
		totalUsdValue.value = formatBalanceWith9Digits(data.total_balance)
		
		// 更新盈亏数据
		profitData.value.value = data.today_profit || 0
		profitData.value.rate = formatPercent(data.today_profit_rate)
		
		// 更新资产列表（使用9位有效数字格式化，并按余额降序排序）
		cryptoAssets.value = (data.assets || [])
			...
			.map(asset => ({
				...
				usdValue: formatBalanceWith9Digits(asset.usd_value)
			}))
	} catch (err) {
		console.error('[Assets] 获取资产数据失败:', err)
	} finally {
