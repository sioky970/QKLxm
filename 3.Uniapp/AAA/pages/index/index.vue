			...
			<view class="navbar-content">
				...
				<view class="navbar-right">
					<view class="icon-btn" @click="goToSettings">
						<SvgIcon name="settings" :size="22" color="#333" />
					</view>
					<view class="icon-btn" @click="goToService">
						<SvgIcon name="customer-service" :size="22" color="#333" />
					</view>
				</view>
			</view>
		</view>
...
import { getAssetOverview, getSevenDaysProfitLoss } from '@/utils/api.js'
import { formatBalanceWith9Digits } from '@/utils/format.js'
...
onMounted(() => {
	...
	if (isLoggedIn.value) {
		fetchAssetData()
		// 不再调用fetchMarketData()和订阅WebSocket
		// 直接使用全局marketStore的数据
	}
})

// 页面显示时检查登录状态并刷新资产数据，但不再重新获取市场数据
onShow(() => {
	checkLoginStatus()
	if (isLoggedIn.value) {
		fetchAssetData()
		// 不再调用 fetchMarketData()，避免显示预设价格
		// WebSocket会持续推送最新数据
	}
})
...
const fetchAssetData = async () => {
	...
	console.log('[Index] 开始获取资产数据...')
	console.log('[Index] 当前用户信息:', userInfo)
	console.log('[Index] 用户ID:', userInfo ? userInfo.id : 'null')
	
	isLoadingAssets.value = true
	try {
		const res = await getAssetOverview()
		console.log('[Index] getAssetOverview 原始响应:', res)
		console.log('[Index] res.data:', res.data)
		
		const data = res.data
		console.log('[Index] 提取的data对象:', data)
		console.log('[Index] total_balance字段:', data.total_balance)
		console.log('[Index] total_usd_value字段:', data.total_usd_value)
		console.log('[Index] today_profit字段:', data.today_profit)
		console.log('[Index] today_profit_rate字段:', data.today_profit_rate)
		console.log('[Index] assets数组长度:', data.assets ? data.assets.length : 0)
		if (data.assets && data.assets.length > 0) {
			console.log('[Index] assets数组示例（前3个）:', data.assets.slice(0, 3))
		}
		
		// 更新资产数据
		assetData.value.totalBalance = formatNumber(data.total_balance)
		// USDT价值按1:1兑换为美元
		assetData.value.totalUsdValue = formatNumber(data.total_balance)
		console.log('[Index] 更新后的totalBalance:', assetData.value.totalBalance)
		console.log('[Index] 更新后的totalUsdValue:', assetData.value.totalUsdValue)
		
		// 处理今日盈亏
		const profit = data.today_profit
		const profitRate = data.today_profit_rate
		
		if (profit >= 0) {
			assetData.value.todayProfit = `+${formatNumber(profit)}`
			assetData.value.profitClass = 'up'
			assetData.value.profitColor = '#00C087'
		} else {
			assetData.value.todayProfit = formatNumber(profit)
			assetData.value.profitClass = 'down'
			assetData.value.profitColor = '#FF4444'
		}
		assetData.value.profitRate = profitRate
		console.log('[Index] 更新后的盈亏数据:', {
			todayProfit: assetData.value.todayProfit,
			profitRate: assetData.value.profitRate,
			profitClass: assetData.value.profitClass
		})
		
		// 获取七日盈亏数据用于折线图
		fetchSevenDaysData()
	} catch (err) {
		console.error('[Index] ❌ 获取资产数据失败:', err)
		uni.showToast({
			title: err.message || '获取资产数据失败',
			icon: 'none'
		})
	} finally {
		isLoadingAssets.value = false
		console.log('[Index] 资产数据获取流程结束')
	}
}
...
const quickActions = ref([
	{ id: 'flash-swap', label: '闪兑', sublabel: '', icon: '/static/crypto-icons/flash-swap.svg', hot: true },
