...
import { formatCurrency, formatNumber, formatPriceWithPrecision } from '@/utils/format.js'
...
const marketList = computed(() => {
	...
	return allData.map(data => {
		...
		return {
			...
			priceNum: priceNum,
			usd: formatPriceWithPrecision(priceNum, precision),
...
onShow(() => {
	...
	if (!wsClient.isConnected) {
		console.log('[Market] WebSocket 未连接，尝试重连')
		wsClient.connect()
	}
	
	// 使用 marketStore 的防抖机制刷新数据
	// fetchMarketData 内部会检查是否需要刷新（5秒防抖）
