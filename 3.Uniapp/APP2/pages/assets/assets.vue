					...
					<view class="wallet-balance-row">
						...
						<view class="wallet-balance-item" @tap="navigateToTransfer('fund')">
							...
							<view class="wallet-balance-right">
								<text class="wallet-balance-amount">{{ isAssetsVisible ? fundBalance : '****' }} USDT</text>
...
const fundBalance = computed(() => formatBalanceWith9Digits(walletStore.state.fundBalance))
...
// 跳转到划转页面
const navigateToTransfer = (walletType) => {
	console.log('[Transfer] 跳转到划转页面，钱包类型:', walletType)
	uni.navigateTo({ 
		url: `/pages/wallet/transfer?walletType=${walletType}` 
	})
}
</script>
