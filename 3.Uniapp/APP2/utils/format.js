 ...
 * @param {number} precision - 精度（小数位数），默认4
 * @returns {string} 格式化后的价格字符串（自动去除尾随零）
 */
export function formatPriceWithPrecision(price, precision = 4) {
	...
	// 使用toFixed格式化后，转为数字再转字符串，自动去除尾随零
	return parseFloat(num.toFixed(safePrecision)).toString();
}

/**
 * 智能格式化价格（根据价格大小自动选择精度）
 * 用于不知道币种精度配置时的降级处理
 * @param {number|string} price - 价格
 * @returns {string} 格式化后的价格字符串（自动去除尾随零）
 */
export function formatPriceSmart(price) {
	const num = parseFloat(price);
	if (typeof num !== 'number' || isNaN(num)) {
		return '0';
	}
	...
	let precision;
	if (num >= 10000) {
		precision = 2;      // BTC等高价币: 2位
	} else if (num >= 100) {
		precision = 2;      // ETH, BNB等: 2位
	} else if (num >= 1) {
		precision = 4;      // 中价币: 4位
	} else if (num >= 0.01) {
		precision = 4;      // 低价币: 4位
