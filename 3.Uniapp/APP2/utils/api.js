...
const PROD_URL = 'https://api.gamaoxo223.shop/api'
...
const responseInterceptor = (response) => {
	...
	if (statusCode === 200) {
		// 后端响应格式: {type: "success/ok/error", message: "xxx", error: "", data: {...}}
		// type为"success"或"ok"表示成功，"error"表示业务错误
		if (data && (data.type === 'success' || data.type === 'ok')) {
...
export const walletTransferApi = {
	 ...
	 * @param {string} data.from_wallet - 转出钱包: spot(现货) / contract(永续合约) / delivery(交割合约) / fund(资金账户)
	 * @param {string} data.to_wallet - 转入钱包: spot(现货) / contract(永续合约) / delivery(交割合约) / fund(资金账户)
	...
	/**
	 * 获取资金账户余额
	 * @returns {Promise<{available_balance: string, locked_balance: string, total_balance: string}>}
	 */
	getFundBalance() {
		return request({
			url: '/wallet/fund/balance',
			method: 'GET'
		})
	}
}
 ...
 * @param {number} data.currency_id - 币种ID
 * @param {string} data.direction - 方向: "rise"(开多) 或 "fall"(开空)
