...
func SetupAdminRoutes(r *gin.RouterGroup) {
	...
	admin.Use(middleware.AdminJWTAuth())
	{
		...
		wallet := admin.Group("/wallet")
		{
			...
			wallet.POST("/withdrawals", adminHandler.GetWithdrawalsList)
			wallet.GET("/withdrawals/:id", adminHandler.GetWithdrawalDetail)
			wallet.POST("/withdrawals/approve", adminHandler.ApproveWithdrawal)
			wallet.POST("/withdrawals/reject", adminHandler.RejectWithdrawal)
			wallet.POST("/charge/list", adminHandler.GetChargeList)
			wallet.POST("/charge/approve", adminHandler.ApproveCharge)
			wallet.POST("/charge/reject", adminHandler.RejectCharge)
		}
		*/
