...
func ConfirmLegalReceive(c *gin.Context) {
	...
	if err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.UsersWallet{}).
			Where("user_id = ? AND currency = ?", deal.UserID, send.CurrencyID).
			Update("legal_balance", gorm.Expr("legal_balance + ?", deal.Number)).Error; err != nil {
			return err
		}
...
func BackC2CSend(c *gin.Context) {
	...
	if err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if send.SurplusNumber > 0 {
			return tx.Model(&model.UsersWallet{}).
				Where("user_id = ? AND currency = ?", send.SellerID, send.CurrencyID).
				Update("change_balance", gorm.Expr("change_balance + ?", send.SurplusNumber)).Error
		}
