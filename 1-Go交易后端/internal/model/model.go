...
// UsersWallet 用户钱包表（已废弃，仅保留兼容性）
// 注意：实际资产数据应从 UserAssets 表读取
type UsersWallet struct {
	...
	// USDT余额（保留用于兼容旧代码，实际使用 UserAssets 表）
...
func (UsersWallet) TableName() string {
	return "users_wallet"
}
...
type UserAssets struct {
	ID     uint `gorm:"primaryKey;column:id" json:"id"`
	UserID uint `gorm:"uniqueIndex;column:user_id;not null" json:"user_id"`
