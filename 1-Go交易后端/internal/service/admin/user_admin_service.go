...
func (s *UserAdminService) AdjustBalance(userID uint, currencyID uint, balanceType string, amount float64, reason string) error {
	return database.DB.Transaction(func(tx *gorm.DB) error {
		...
		// 2. 检查余额是否足够
		if wallet.UsdtBalance+amount < 0 {
			return errors.New("USDT余额不足")
		}

		// 3. 更新余额(旧表)
		if err := tx.Model(&wallet).Update("usdt_balance", gorm.Expr("usdt_balance + ?", amount)).Error; err != nil {
			return err
		}

		// 3.5. 同步到新表 user_assets (双写模式)
		assetsService := service.GetUserAssetsService()
		err := assetsService.UpdateUsdtBalance(userID, amount, false)
		if err != nil {
			logger.Warnf("[DoubleWrite] 同步到 user_assets 失败: userID=%d, amount=%.8f, err=%v", userID, amount, err)
			// 不阻塞主流程,仅记录日志
		}

		// 4. 记录日志
		logType := 100 // 类型100表示统一余额调整
		if amount > 0 {
			logType = 101 // 增加
		} else {
			logType = 102 // 减少
		...
		accountLog := model.AccountLog{
			UserID:      userID,
			Value:       amount,
			CurrencyID:  currencyID, // 使用CurrencyID
			Type:        logType,
			Info:        reason,
		...
		logger.Infof("用户 %d USDT余额调整: 变动=%.8f, 新余额=%.8f, 原因=%s",
			userID, amount, wallet.UsdtBalance+amount, reason)
...
func (s *UserAdminService) SearchUsers(keyword, realName string, status *int, isReal *int, risk *int8, page, pageSize int) (list []UserListItem, total int64, err error) {
	db := database.DB.Table("users AS u").
		Select("u.*, ur.name AS real_name, ur.card_id, p.account_number AS parent_account, w.usdt_balance, w.lock_usdt_balance").
		Joins("LEFT JOIN user_real ur ON ur.user_id = u.id").
		Joins("LEFT JOIN users p ON u.parent_id = p.id").
		Joins("LEFT JOIN users_wallet w ON w.user_id = u.id AND w.currency = 3") // currency=3 是USDT
