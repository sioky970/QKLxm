...
func (e *MatchingEngine) Start() {
	...
	for {
		...
		case <-e.stopChan:
			logger.Info("[MatchingEngine] 价格监控任务已停止")
			e.running = false
			return
		}
	}
...
func (e *MatchingEngine) executeOrder(order *model.Transaction, fillPrice float64) error {
	...
	// 获取用户钱包
	wallet, err := GetWalletService().GetWalletByCurrency(order.FromUserID, order.LegalID)
	if err != nil {
		tx.Rollback()
	...
	if order.Type == 1 { // 买入：USDT → 加密货币
		// 1. 从锁定的USDT中扣除
		if err := tx.Model(&model.UsersWallet{}).Where("id = ?", wallet.ID).Updates(map[string]interface{}{
			"lock_usdt_balance": gorm.Expr("lock_usdt_balance - ?", tradeAmount),
		}).Error; err != nil {
			tx.Rollback()
		...
		// 1. 从锁定的币种中扣除，增加USDT可用余额
		if err := tx.Model(&model.UsersWallet{}).Where("id = ?", wallet.ID).Updates(map[string]interface{}{
			"lock_usdt_balance": gorm.Expr("lock_usdt_balance - ?", order.Number),
			"usdt_balance":      gorm.Expr("usdt_balance + ?", tradeAmount),
		}).Error; err != nil {
			tx.Rollback()
			logger.Errorf("[MatchingEngine] 更新卖出余额失败: %v", err)
			return err
		}
	...
	// WebSocket推送余额更新
	go GetWalletService().BroadcastBalanceUpdate(order.FromUserID)

	// WebSocket推送订单状态变更
	go GetWalletService().BroadcastOrderUpdate(order.FromUserID, order.ID, 2, order.Number, fillPrice)
