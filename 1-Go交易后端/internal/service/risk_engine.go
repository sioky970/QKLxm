...
func (e *RiskEngine) CalculateMicroRiskResult(order *model.MicroOrder) MicroRiskResult {
	...
	if order.UserID > 0 {
		...
		if err == nil && userRisk != types.RiskResultNone {
			logger.Debugf("秒合约风控[账户]: userID=%d, result=%d", order.UserID, userRisk)
	...
	if order.CurrencyID > 0 {
		...
		if currencyRisk != MicroRiskNone {
			logger.Debugf("秒合约风控[币种]: currencyID=%d, result=%d", order.CurrencyID, currencyRisk)
			return currencyRisk
		}
	}
	...
	if globalRisk != MicroRiskNone {
		logger.Debugf("秒合约风控[全局]: result=%d", globalRisk)
		return globalRisk
	}

	// 4) 默认: 50%概率随机盈亏
	result := e.defaultRandom()
	logger.Debugf("秒合约风控[默认50%%]: result=%d", result)
	return result
}
