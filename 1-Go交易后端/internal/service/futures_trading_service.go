...
func (s *FuturesTradingService) OpenPosition(userID uint, req *OpenPositionRequest) (*OpenPositionResponse, error) {
	...
	err = database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.UsersWallet{}).
			Where("id = ?", wallet.ID).
			Update("usdt_balance", gorm.Expr("usdt_balance - ?", req.Margin)).Error; err != nil {
...
func (s *FuturesTradingService) CancelOrder(userID uint, orderID uint) error {
	...
	err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.UsersWallet{}).
			...
			Update("usdt_balance", gorm.Expr("usdt_balance + ?", position.OriginCautionMoney)).Error; err != nil {
	...
	err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if returnValue > 0 {
			if err := tx.Model(&model.UsersWallet{}).
				...
				Update("usdt_balance", gorm.Expr("usdt_balance + ?", returnValue)).Error; err != nil {
				return err
			}
		}
