...
func (s *MicroTradingService) SubmitOrder(userID uint, req *MicroSubmitOrderRequest) (*MicroSubmitOrderResponse, error) {
	...
	// 9. 验证用户USDT余额
	wallet, err := GetWalletService().GetWalletByCurrency(userID, legalID)
	if err != nil {
		return nil, errors.New("获取钱包失败")
	}
	if wallet.UsdtBalance < req.Amount {
		return nil, errors.New("USDT余额不足")
	}
	...
	// 12. 锁定投入金额（仅本金，无手续费）
	result := tx.Model(&model.UsersWallet{}).
		Where("user_id = ? AND currency = ? AND usdt_balance >= ?", userID, legalID, req.Amount).
		Updates(map[string]interface{}{
			"usdt_balance":      gorm.Expr("usdt_balance - ?", req.Amount),
			"lock_usdt_balance": gorm.Expr("lock_usdt_balance + ?", req.Amount),
		})
	if result.Error != nil || result.RowsAffected == 0 {
		tx.Rollback()
		return nil, errors.New("锁定资金失败，余额可能已变化")
	}
