  ...
  <div class="withdrawal-container">
    ...
    <a-card class="table-card" :bordered="false" style="margin-top: 16px">
      ...
      >
        ...
        <template #withdraw_type="{ record }">
          <a-tag v-if="record.withdraw_type === 2" color="purple">
          ...
          <div v-if="record.withdraw_type === 2 && record.network_type" class="network-tag">
            {{ record.network_type }}
          </div>
        </template>
    ...
    >
      <!-- 区块链提币提示 -->
      <a-alert v-if="approveForm.withdraw_type === 2" type="info" style="margin-bottom: 16px">
        <template #icon><icon-send /></template>
        区块链提币 - {{ approveForm.network_type }}，请核对钱包地址后进行打款
      </a-alert>
      <!-- 银行转账提示 -->
      <a-alert v-else type="info" style="margin-bottom: 16px">
        请仔细核对用户实名信息与银行卡信息后再进行审核
      </a-alert>

      <!-- 实名信息展示（银行转账时） -->
      <a-descriptions
        v-if="approveForm.withdraw_type !== 2 && (approveForm.real_name || approveForm.id_card)"
        title="实名信息快照"
        :column="2"
        bordered
      ...
      >
        <a-descriptions-item label="真实姓名">
          {{ approveForm.real_name || '-' }}
        </a-descriptions-item>
        <a-descriptions-item label="身份证号">
          {{ maskIDCard(approveForm.id_card) }}
        </a-descriptions-item>
      </a-descriptions>

      <!-- 银行卡信息展示（银行转账时） -->
      <a-descriptions
        v-if="approveForm.withdraw_type !== 2 && (approveForm.bank_name || approveForm.bank_account)"
        title="收款银行卡信息"
        :column="2"
        bordered
        style="margin-bottom: 16px"
      ...
      <!-- 区块链地址信息（区块链提币时） -->
      <a-descriptions
        v-if="approveForm.withdraw_type === 2"
        title="区块链收款地址"
        :column="1"
        bordered
        style="margin-bottom: 16px"
      >
        <a-descriptions-item label="网络类型">
          <a-tag color="purple">{{ approveForm.network_type || '-' }}</a-tag>
        </a-descriptions-item>
        <a-descriptions-item label="钱包地址">
          <span style="font-family: monospace; word-break: break-all; color: #165dff">
            {{ approveForm.chain_address || '-' }}
          </span>
          <a-button 
            v-if="approveForm.chain_address" 
            type="text" 
            size="mini" 
            @click="copyText(approveForm.chain_address)"
          >
            <template #icon><icon-copy /></template>
          </a-button>
        </a-descriptions-item>
    ...
    >
      <!-- 提现类型标识 -->
      <a-alert 
        v-if="(detailData as any).withdraw_type === 2" 
        type="info" 
        style="margin-bottom: 16px"
      >
        <template #icon><icon-send /></template>
        区块链提币 - {{ (detailData as any).network_type || 'USDT' }}
      </a-alert>
      <a-alert 
        v-else 
        type="normal" 
      ...
      <!-- 实名信息快照（银行转账时显示） -->
      <a-descriptions
        v-if="(detailData as any).withdraw_type !== 2 && ((detailData as any).real_name || (detailData as any).id_card)"
        title="实名信息快照"
        :column="2"
        bordered
        style="margin-bottom: 16px"
      >
        <a-descriptions-item label="真实姓名">
          {{ (detailData as any).real_name || '-' }}
        </a-descriptions-item>
        <a-descriptions-item label="身份证号">
          {{ maskIDCard((detailData as any).id_card) }}
        </a-descriptions-item>
      </a-descriptions>

      <!-- 银行卡信息快照（银行转账时显示） -->
      <a-descriptions
        v-if="(detailData as any).withdraw_type !== 2 && ((detailData as any).bank_name || (detailData as any).bank_account)"
        title="收款银行卡信息"
        :column="2"
        bordered
        style="margin-bottom: 16px"
      >
        <a-descriptions-item label="开户银行">
          {{ (detailData as any).bank_name || '-' }}
        </a-descriptions-item>
        <a-descriptions-item label="开户支行">
          {{ (detailData as any).bank_branch || '-' }}
        </a-descriptions-item>
        <a-descriptions-item label="银行卡号" :span="2">
          <span style="font-family: monospace; letter-spacing: 2px">
            {{ (detailData as any).bank_account || '-' }}
          </span>
        </a-descriptions-item>
        <a-descriptions-item label="持卡人">
          {{ (detailData as any).real_name || '-' }}
      ...
      >
        <a-descriptions-item label="网络类型">
          <a-tag color="purple">{{ (detailData as any).network_type || '-' }}</a-tag>
        </a-descriptions-item>
        <a-descriptions-item label="钱包地址">
          <span style="font-family: monospace; word-break: break-all">
            {{ (detailData as any).chain_address || '-' }}
          </span>
          <a-button 
            v-if="(detailData as any).chain_address" 
            type="text" 
            size="mini" 
            @click="copyText((detailData as any).chain_address)"
          >
            <template #icon><icon-copy /></template>
          </a-button>
        </a-descriptions-item>
      ...
      <a-descriptions :column="2" bordered>
        ...
        <a-descriptions-item label="提现类型">
          <a-tag v-if="(detailData as any).withdraw_type === 2" color="purple">区块链提币</a-tag>
          <a-tag v-else color="blue">银行转账</a-tag>
        </a-descriptions-item>
...
<script setup lang="ts">
  ...
  const approveForm = reactive({
    ...
    bank_account: '',
    // 区块链提币相关
    withdraw_type: 1 as number, // 1=银行转账, 2=区块链提币
    network_type: '',
    chain_address: '',
  });
  ...
  const handleApprove = (record: WithdrawalItem) => {
    Object.assign(approveForm, {
      ...
      notes: '',
      // 实名信息快照
      real_name: (record as any).real_name || '',
      id_card: (record as any).id_card || '',
      // 银行卡信息快照
      bank_name: (record as any).bank_name || '',
      bank_branch: (record as any).bank_branch || '',
      bank_account: (record as any).bank_account || '',
      // 区块链提币相关
      withdraw_type: (record as any).withdraw_type || 1,
      network_type: (record as any).network_type || '',
      chain_address: (record as any).chain_address || '',
    });
