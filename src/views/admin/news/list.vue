<template>
  <div class="news-list-container">
    ...
    >
      <a-form :model="newsForm" :label-col-props="{ span: 4 }" :wrapper-col-props="{ span: 20 }">
        <a-row :gutter="16">
          <a-col :span="24">
            <a-form-item label="标题" required>
              <a-input v-model="newsForm.title" placeholder="请输入新闻标题" :max-length="100" show-word-limit />
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="作者">
              <a-input v-model="newsForm.author" placeholder="请输入作者" :max-length="50" show-word-limit />
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="状态">
              <a-radio-group v-model="newsForm.status" type="button">
          ...
          <a-col :span="24">
            <a-form-item label="摘要">
              <a-textarea
                ...
                placeholder="请输入新闻摘要（选填，不填将自动从内容提取）"
                :max-length="200"
                show-word-limit
              />
            </a-form-item>
...
<script setup lang="ts">
  ...
  const handleNewsEdit = (record: any) => {
    ...
    Object.assign(newsForm, {
      ...
      publish_date: record.publish_time ? dayjs.unix(record.publish_time).toDate() : new Date(),
    });
    newsModalVisible.value = true;
  };
  ...
  const handleNewsSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        abstract: newsForm.abstract,
        status: newsForm.status,
        publish_time: dayjs(newsForm.publish_date).unix(),
      };
      if (newsForm.id) {
