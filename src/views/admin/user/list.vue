          ...
          <a-row :gutter="16">
            ...
            <a-col :span="12">
              <a-form-item label="支付密码">
                <a-input
                  :model-value="editForm.has_pay_password ? '已设置' : '未设置'"
                  disabled
                  :style="{
                    color: editForm.has_pay_password
                      ? 'rgb(var(--green-6))'
                      : 'var(--color-text-3)',
                  }"
                />
            ...
            <a-col :span="12">
              <a-form-item label=" ">
                <a-button
                  ...
                  status="warning"
                  size="small"
                  :disabled="!editForm.has_pay_password"
                  @click="handleClearPayPassword"
                >
                  <template #icon><icon-delete /></template>
                  清除支付密码
                </a-button>
      ...
      <a-form :model="adjustBalanceForm" layout="vertical">
        ...
        <a-form-item label="钱包类型" required>
          <a-radio-group v-model="adjustBalanceForm.wallet_type">
            ...
            <a-radio value="fund">
              <a-tag color="cyan" size="small">资金账户</a-tag>
            </a-radio>
          </a-radio-group>
        </a-form-item>
...
<script setup lang="ts">
  ...
  const editForm = reactive({
    ...
    password: '',
    status: 0,
    has_pay_password: false, // 是否设置了支付密码
  });
  ...
  const handleEdit = (record: any) => {
    Object.assign(editForm, {
      ...
      has_pay_password: !!record.pay_password, // 判断是否设置了支付密码
    });
  ...
  // 清除支付密码
  const handleClearPayPassword = () => {
    Modal.confirm({
      title: '确认清除支付密码',
      content: `确定要清除用户 ${editForm.account_number} 的支付密码吗？清除后用户需要重新设置！`,
      okText: '确认清除',
      cancelText: '取消',
      okButtonProps: { status: 'warning' },
      onOk: async () => {
        try {
          await axios.post('/admin/user/clear-pay-password', {
            id: editForm.id,
          });
          Message.success('支付密码已清除');
          editForm.has_pay_password = false;
          fetchUserList(); // 刷新列表
        } catch (error: any) {
          Message.error(error.message || '清除支付密码失败');
        }
      },
    });
  };
      ...
      onOk: async () => {
        try {
          ...
          await axios.post('/admin/user/adjust-balance', {
            ...
            currency_id: 1, // USDT 的 currency_id (与钱包系统一致)
            wallet_type: adjustBalanceForm.wallet_type, // spot 或 contract
            amount: actualAmount,
